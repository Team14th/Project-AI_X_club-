...
from app.models import BorrowRecord, Employee, Tool  # 新增：导入Tool模型
...
def init_tool():
    ...
    db = SessionLocal()  # 创建数据库会话
    try:
        tool = db.query(Tool).filter(Tool.id == 1).first()  # 查询ID=1的工具

        if not tool:  # 如果没有工具，创建默认工具
            tool = Tool(
                id=1,  # 固定工具ID为1（和其他代码匹配）
                name="车间通用扳手",  # 工具名称
                total_quantity=10,  # 总数量
                current_quantity=10,  # 当前库存
                threshold=2,  # 低库存预警阈值
                unit_weight=100.0,  # 单工具重量（g）
                status="正常",  # 初始状态
                last_updated=datetime.now()  # 更新时间
            )
            db.add(tool)  # 添加到数据库会话
            db.commit()  # 提交保存
            logger.info("工具数据初始化完成：默认创建10把车间通用扳手")
    finally:
        db.close()  # 关闭数据库会话
...
# 以下原有函数（create_return_record、get_all_records等）保持不变，无需修改
